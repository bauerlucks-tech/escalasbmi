...
// Schema completo para o sistema de escalas
...
CREATE TABLE month_schedules (
  ...
  entries JSONB NOT NULL, -- Array de ScheduleEntry
...
-- 3. TABELA DE ENTRADAS DE ESCALA (para consultas otimizadas)
...
-- 4. TABELA DE SOLICITAÇÕES DE TROCA
...
CREATE POLICY "Admins can view all users" ON users
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() AND role IN ('administrador', 'super_admin')
    )
  );

-- Todos podem ver escalas ativas
CREATE POLICY "Anyone can view active schedules" ON month_schedules
  FOR SELECT USING (is_active = TRUE);

-- Admins podem gerenciar escalas
CREATE POLICY "Admins can manage schedules" ON month_schedules
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() AND role IN ('administrador', 'super_admin')
...
-- 11. VIEWS ÚTEIS
-- View para escalas ativas com detalhes
...
-- 14. SUBSCRIPTIONS PARA REAL-TIME
-- Os clientes podem se inscrever em:
-- - month_schedules (para mudanças nas escalas)
-- - swap_requests (para novas solicitações de troca)
-- - vacation_requests (para novas solicitações de férias)
-- - audit_logs (para auditoria em tempo real)

-- Exemplo de subscription no cliente:
-- supabase.channel('scales').on('postgres_changes', 
--   { event: '*', schema: 'public', table: 'month_schedules' }, 
--   (payload) => { console.log('Mudança na escala:', payload); }
-- ).subscribe()
