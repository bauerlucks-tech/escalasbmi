...
import { Upload, FileSpreadsheet, AlertTriangle, CheckCircle, XCircle, Calendar, Users, ArrowLeft, Edit3, Save, X } from 'lucide-react';
...
const CSVImportPage: React.FC = () => {
  ...
  const handleEditError = (error: CSVParsedData) => {
    setEditingErrors([...editingErrors, { ...error }]);
  };

  const handleSaveError = (index: number, updated: CSVParsedData) => {
    const newErrors = [...editingErrors];
    newErrors[index] = updated;
  ...
  const handleApplyCorrections = () => {
    if (editingErrors.length === 0) {
      toast.error('Nenhuma correção para aplicar');
      return;
    }

    // Remove original errors from csvData
    const updatedCsvData = csvData.filter(item => !item.error);
    
    // Add corrected items
    const correctedItems = editingErrors.map(item => {
      const errors: string[] = [];
      ...
      if (!validatePost(item.post)) {
        errors.push('Posto inválido: use meio_periodo ou fechamento');
      }
      
      if (!item.collaborator || item.collaborator.length < 2) {
        errors.push('Colaborador inválido: mínimo 2 caracteres');
      }

      return {
        ...item,
        post: item.post.toLowerCase(),
        collaborator: item.collaborator.toUpperCase(),
        error: errors.length > 0 ? errors.join('; ') : undefined
      };
    });

    const newCsvData = [...updatedCsvData, ...correctedItems];
    setCsvData(newCsvData);
    setParsedMonths(groupByMonth(newCsvData));
    setEditingErrors([]);
    
    const remainingErrors = newCsvData.filter(item => item.error).length;
    if (remainingErrors === 0) {
      toast.success('Todas as correções aplicadas com sucesso!');
    } else {
      toast.warning(`Correções aplicadas, mas ainda existem ${remainingErrors} erro(s).`);
    }
  };

  const handleImportMonth = (month: MonthPreview) => {
    if (month.errors.length > 0) {
      toast.error('Corrija os erros antes de importar este mês');
      return;
    }

    // Here you would integrate with your existing import system
    // For now, we'll just show a success message
    toast.success(`Mês ${month.monthName} ${month.year} importado com sucesso!`);
    
    // TODO: Integrate with existing importNewSchedule function
    // importNewSchedule(month.month, month.year, month.days, currentUser.name, true);
  };
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Importação de Escalas CSV</h1>
        ...
        {editingErrors.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Corrija os erros encontrados antes de importar
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {editingErrors.map((error, index) => (
                    <div key={index} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Linha {error.line}</span>
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <input
                            type="text"
                            value={error.date}
                            onChange={(e) => handleSaveError(index, { ...error, date: e.target.value })}
                        ...
                        <div>
                          ...
                          <select
                            value={error.post}
                            onChange={(e) => handleSaveError(index, { ...error, post: e.target.value })}
                            className="w-full p-2 border rounded text-sm"
                          >
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={error.collaborator}
                            onChange={(e) => handleSaveError(index, { ...error, collaborator: e.target.value })}
                            className="w-full p-2 border rounded text-sm"
                            placeholder="NOME"
                          />
                        </div>
              ...
              <div className="flex gap-2 mt-4">
                <Button onClick={handleApplyCorrections} className="flex-1">
                  <Save className="w-4 h-4 mr-2" />
                  Aplicar Correções
                </Button>
                <Button variant="outline" onClick={() => setEditingErrors([])}>
                  Cancelar
                </Button>
              </div>
            </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="max-h-32 overflow-y-auto space-y-1">
                            {month.errors.map((error, index) => (
                              <div key={index} className="text-xs bg-destructive/10 p-2 rounded flex items-center justify-between">
                                <span>Linha {error.line}: {error.error}</span>
                                <Button
                                  variant="ghost"
                                  size="sm"
                      ...
                      <Button
                        ...
                        disabled={month.errors.length > 0}
                        className="w-full"
                      >
                        <Users className="w-4 h-4 mr-2" />
                        Importar {month.monthName}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
